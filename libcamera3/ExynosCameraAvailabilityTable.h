...
static uint8_t AVAILABLE_EDGE_MODES[] =
{
    ...
    /* NOT supported on this pipe generation, do not advertise */
...
{
    ANDROID_LENS_OPTICAL_STABILIZATION_MODE_OFF,
    /* j5y17lte rear IMX258 has no OIS hardware - do not advertise it */
    //ANDROID_LENS_OPTICAL_STABILIZATION_MODE_ON,
};
...
static uint8_t AVAILABLE_CAPABILITIES_LIMITED[] =
{
    ...
    /*
     * Not supported on the 34xx stack for exynos7870 (table was ported
     * verbatim from 74xx):
     * - RAW: 34xx hal3 never assembles RAW stream configs (no RAW16/RAW_SENSOR
     *   handling in ExynosCamera3Parameters.cpp) - apps would see the RAW
     *   capability with zero raw sizes.
     * - CONSTRAINED_HIGH_SPEED_VIDEO: high-speed configs use the FPS60/120/240
     *   HWDIS buffer formulas (37/96/96 buffers > the 3.18 driver's 32-buf
     *   cap) - every high-speed session dies at reqBuffers.
     */
...
{
    /*
     * Body restored to the 74xx original: empty. The previous body carried
     * ANDROID_REQUEST_AVAILABLE_CAPABILITIES_* *capability* constants instead
     * of request keys (copy/paste accident from CAPABILITIES_LIMITED); LEGACY
     * level requires no mandatory request keys.
     */
};
...
static uint8_t AVAILABLE_FACE_DETECT_MODES[] =
{
    ...
    /* FULL would require FACE_LANDMARKS results, which the VRA/HW path never
       delivers (the landmarks key is not part of the result keys) */
...
/* -------------------------------------------------------------------------
 * Availability arrays required by the common_v2 camera3 sensor info code.
 * Ported verbatim from the 74xx (v2-era) ExynosCameraAvailabilityTable.h;
 * values may need device-specific tuning for exynos7870.
 */
